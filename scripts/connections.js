    ...
    icons.forEach((iconA, index) => {
        icons.forEach((iconB, index2) => {
            if (index !== index2) {
                ...
                line.style.width = `${4 - (length / Math.max(container.clientWidth, container.clientHeight))}px`;
...
function startConnectionsAnimation() {
    if (!isAnimating) {
        icons = document.querySelectorAll('.floatingIcon');
        container = document.querySelector('.networkContainer');
        isAnimating = true;
        animate();
    }
}

function stopConnectionsAnimation() {
    isAnimating = false;
